...
void main() {
  ...
  test('alimentar sube el hambre y persiste', () async {
    await MascotaService.obtenerEstado();
    await MascotaService.alimentar(); // desde 100 no puede subir más, clampeado
    ...
    final e = await MascotaService.alimentar(suba: 30);
    expect(e.hambre, closeTo(70, 0.5));
  });

  test('alternarDormir cambia el estado y se persiste', () async {
    await MascotaService.obtenerEstado();
    final dormida = await MascotaService.alternarDormir(true);
    expect(dormida.durmiendo, true);
    final despierta = await MascotaService.alternarDormir(false);
  ...
  test('registrarJuego sube la diversión', () async {
    ...
    final e = await MascotaService.registrarJuego(suba: 15);
