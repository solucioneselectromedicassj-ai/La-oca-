...
/// 6 oca, 6 minijuego, 3 puente, 3 cárcel, 2 calavera. El resto quedan
/// normales.
///
...
class BoardLayout {
  BoardLayout._(this.casillas, this.puentes);

  static const int meta = 29;

  final Map<int, TipoCasilla> casillas;
  final Map<int, InfoPuente> puentes;
  ...
  InfoPuente? infoPuenteDe(int posicion) => puentes[posicion];
  ...
  factory BoardLayout.generar({Random? random}) {
    ...
    for (final entry in reparto.entries) {
      for (var i = 0; i < entry.value; i++) {
        ...
        if (entry.key == TipoCasilla.puente) {
          puentes[posicion] = InfoPuente(avance: 2 + rng.nextInt(3)); // 2-4
        }
      }
    }

    return BoardLayout._(casillas, puentes);
  }
}
